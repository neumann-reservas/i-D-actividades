...
const WEB_APP_URL = "https://script.google.com/macros/s/AKfycbzILCLRZVQKvfgRZaSOHNvo8r6Rv7kb8bvsY7eANMf-tQfs9FKvI6J523kW_OEMrDuS/exec";
const MAX_IMAGES = 5;
let allReportsData = [], executivesData = [], uploadedFiles = [], quill;
...
function createTable(data) {
    ...
    data.forEach((row, index) => {
        ...
        const statusSelectHtml = `
            <select class="form-select form-select-sm status-select status-${estado}" data-id="${reportId}">
        ...
        tableHtml += `<tr>
            ...
            <td class="text-center">
                <button class="btn btn-sm btn-outline-secondary view-btn" data-index="${index}" title="Ver Detalles">
...
// --- PUNTO DE ENTRADA DEL SCRIPT: EVENTO DOMContentLoaded ---
document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- ¡NUEVO EVENTO DE PEGADO INTELIGENTE! ---
    document.getElementById('reportModal').addEventListener('paste', (event) => {
        ...
        if (foundImage) {
            event.preventDefault(); // Prevenimos que se pegue en el editor
        ...
        // Si no es una imagen, no hacemos nada y dejamos que el editor maneje el pegado de texto de forma nativa.
    });
    ...
    $('#dataTableContainer').on('change', '.status-select', async function() {
        ...
        try {
            await appFetch('updateStatus', { reportId: reportId, newStatus: newStatus });
            $(this).removeClass('status-Enviado status-Recibido status-Observado').addClass(`status-${newStatus}`);
            ...
            $(this).val(allReportsData.find(row => row[0] == reportId)[11]);
        }
    });
});